...
const hyperIdSignalCollector = () => {
    return new Promise((resolve, reject) => {
        resolve("ss test: " + hyperId);
    }).catch(function() {
        console.log("Got error from signal collector")
    });
}
    ...
    googletag.cmd.push(function () {
        ...
        googletag.secureSignalProviders.push({
         ...
         if (typeof googletag.secureSignalProviders.clearAllCache === 'function') {
             console.log(googletag.secureSignalProviders.clearAllCache);
             googletag.secureSignalProviders.clearAllCache().then(function() {
                 console.log("Cache cleared successfully.");
             }).catch(function(error) {
                 console.error("An error occurred while clearing cache: ", error.message);
             });
         }
    });
}
...
function startAds(data, callback) {
    ...
    // googletag.cmd.push(function () {
    //     var slot = googletag.defineSlot('/novatiq/test', [300, 250], '/18339668/test-delivery/test_openbidding').addService(googletag.pubads());
    //     if (data != null && data['segments'].length > 0) {
